...
Two consultation calls were held with industry experts (Dale and Eric) to review this proposal. Key conclusions that shaped this revision:
...
| Coil:Cooling:ITE:ColdPlate | Defines the physical heat transfer between the IT equipment and the standard E+ plant loop for single-phase coolants. | Heat transfer input method (UA or thermal resistance), maximum allowable chip temperature, fluid inlet/outlet nodes, design pressure drop, design maximum flow rate. | Establishes the maximum thermal ceiling, dictating when cooling capacity is maxed out and residual heat must spill over into the zone air. Note: this object is implemented under a separate NFP; this table flags the `Design Maximum Flow Rate` field as a requirement for that implementation, representing the vendor-specified physical flow limit independent of the thermally-derived flow. No IDD for this object is authored here. |
...
Two items raised in review are explicitly not part of this feature:
...
Two adjacent needs, outside the core ITE object but within this NFP's scope, were identified:
...
- A hybrid rack combining a rear-door heat exchanger and direct-to-chip cold plates on the same liquid loop, exercising the Liquid Heat Capture Fraction split.
...
- Consultation calls with industry reviewers Dale and Eric, September 2026 (see E-mail and Conference Call Conclusions).
