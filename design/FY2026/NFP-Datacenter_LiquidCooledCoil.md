...
$$\varepsilon = 1 - e^{-\text{NTU}_{LMTD}}$$

$$T_{case} = T_{in} + \frac{T_{out} - T_{in}}{\varepsilon}$$
...
For both approaches, the coolant outlet temperature is:

$$T_{out} = T_{in} + \frac{\dot{q}}{\dot{m} \cdot c_p}$$

where:
- $T_{out}$ is the coolant outlet temperature
- $T_{in}$ is the coolant inlet temperature
- $\dot{m}$ is the liquid coolant mass flow rate
- $c_p$ is the liquid coolant specific heat capacity at the inlet temperature
...
Coil:Cooling:ITE:ColdPlate,
       \memo Cold plate liquid cooling coil for IT equipment. Models heat transfer from the
       \memo chip case to a plant loop coolant using a thermal resistance approach (Standard
       \memo or LMTD). Pairs with an ElectricEquipment:ITE:LiquidCooled object; heat not
       \memo removed by the coolant is added to the specified zone.
   \min-fields 10
  A1 , \field Name
       \required-field
       \type alpha
  ...
  N3 , \field Target Case Operating Temperature
       \note Targeted operating case temperature, used to size the nominal liquid coolant flow rate.
       \note During variable-flow operation, the flow rate is adjusted to maintain this target temperature.
       \note If not specified, defaults to the Maximum Case Temperature.
       \note The Target Case Operating Temperature cannot exceed the Maximum Case Temperature.
       \type real
       \units C
       \minimum> 0.0
  A5 , \field Thermal Resistance Modifier Curve Name
       \note Optional curve modifying thermal resistance as a function of flow rate ratio (flow / nominal flow).
       \note For univariate curves, flow rate ratio is used as the independent variable.
       \note For bivariate curves, flow rate ratio is the first independent variable
       \note and heat transfer rate ratio (heat transfer rate / nominal heat transfer rate) is the second.
       \note If blank, a modifier of 1.0 is used.
       \type object-list
       \object-list UnivariateFunctions
       \object-list BivariateFunctions
  A6 , \field Thermal Resistance Method
       \note Standard: R = (Tc - Tf,in) / q_dot
       \note where Tc is the chip case temperature, Tf,in is the coolant inlet temperature, and q_dot is the heat transfer rate.
       \note LMTD: R = LMTD / q_dot, where LMTD accounts for the temperature rise of coolant along the cold plate.
       \type choice
       \key Standard
       \key LMTD
       \default Standard
  N4 , \field Nominal Liquid Flow Rate
       \note Nominal volumetric liquid coolant flow rate through the cold plate.
       \type real
       \units m3/s
       \ip-units gal/min
       \minimum> 0.0
  ...
  N5 , \field Maximum Liquid Flow Rate
       \note Maximum volumetric liquid coolant flow rate through the cold plate.
       \note If not specified, the nominal flow rate is used as the upper limit.
       \type real
       \units m3/s
       \ip-units gal/min
       \minimum> 0.0
  ...
  N6 , \field Auxiliary Electric Power
       \note Auxiliary electric power consumed when the cold plate is available, regardless of IT load.
       \type real
       \units W
       \ip-units W
       \minimum 0.0
  ...
  A7 , \field Zone Name
       \required-field
       \note Zone where heat not captured by the cold plate will be added to the zone heat balance.
       \type object-list
       \object-list ZoneNames
  A8 , \field Flow Mode
       \note ConstantFlow: cold plate operates at the nominal liquid flow rate at all times.
       \note VariableFlow: flow rate is adjusted to maintain the target case operating temperature.
       \type choice
       \key ConstantFlow
       \key VariableFlow
...
The following output variables will be added:

| Variable Name | Units | Type |
|---|---|---|
| Coil Cooling ITE Cold Plate Heat Transfer Rate | W | Average |
| Coil Cooling ITE Cold Plate Heat Transfer Energy | J | Sum |
| Coil Cooling ITE Cold Plate Zone Heat Gain Rate | W | Average |
| Coil Cooling ITE Cold Plate Zone Heat Gain Energy | J | Sum |
| Coil Cooling ITE Cold Plate Case Temperature | C | Average |
| Coil Cooling ITE Cold Plate Inlet Temperature | C | Average |
| Coil Cooling ITE Cold Plate Outlet Temperature | C | Average |
