...
SUBROUTINE SetThisVersionVariables()
      ...
      VerString='Conversion 26.2 => 27.1'
      VersionNum=27.1
      ...
      sVersionNum = '***'
      sVersionNumFourChars='27.1'
      IDDFileNameWithPath=TRIM(ProgramPath)//'V26-2-0-Energy+.idd'
      NewIDDFileNameWithPath=TRIM(ProgramPath)//'V27-1-0-Energy+.idd'
      RepVarFileNameWithPath=TRIM(ProgramPath)//'Report Variables 26-2-0 to 27-1-0.csv'
END SUBROUTINE
...
!                 CurArgs = CurArgs + 1
              ...
              CASE('EVAPORATIVECOOLER:DIRECT:RESEARCHSPECIAL')
                  CALL GetNewObjectDefInIDD(ObjectName,NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)
                  nodiff=.false.
                  ObjectName = 'EVAPORATIVECOOLER:DIRECT:USEREFFECTIVENESS'
                  OutArgs(1:CurArgs)=InArgs(1:CurArgs)

              CASE('EVAPORATIVECOOLER:INDIRECT:RESEARCHSPECIAL')
                  CALL GetNewObjectDefInIDD(ObjectName,NwNumArgs,NwAorN,NwReqFld,NwObjMinFlds,NwFldNames,NwFldDefaults,NwFldUnits)
                  nodiff=.false.
                  ObjectName = 'EVAPORATIVECOOLER:INDIRECT:USEREFFECTIVENESS'
                  OutArgs(1:CurArgs)=InArgs(1:CurArgs)
