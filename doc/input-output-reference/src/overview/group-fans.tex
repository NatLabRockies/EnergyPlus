...
\paragraph{Field: Motor Loss Location}\lable{field-motor-loss-location}

Valid choices are Zone, Outside and AirStream. If Zone is selected then a valid zone name must be entered in the Motor Loss Zone Name field below. The motor's thermal losses that are not added to the air stream are added to the surrounding thermal zone as internal heat gains. If Outside is selected then motor losses that are not added to the air stream are directed to outdoors. If AirStream is selected then motor losses are directed to the fan exhaust air stream. In all cases, the amount of motor heat added to the air stream is determined using the following field for Motor in Airstream Fraction which has a default value of 1.0. When AirStream is selected and Motor in Airstream Fraction is less than 1, the motor's thermal losses that are not added to the air stream are directed to outdoors.

\paragraph{Field: Motor in Airstream Fraction}\label{field-motor-in-airstream-fraction-31}

The fraction of the motor heat that is added to the air stream. A value of 0 means that the motor is completely outside the air stream. A value of 1 means that all of the motor heat loss will go into the air stream and act to cause a temperature rise. Must be between 0 and 1. The default is 1.0.

\paragraph{Field: Motor Loss Zone Name}\label{field-loss-zone-name-fanzoneexhaustl}
