...
This object models fans for variable-air-volume, constant-volume, on-off cycling, two-speed, and multi-speed applications. It was designed as a replacement for \hyperref[fanconstantvolume]{Fan:ConstantVolume}, \hyperref[fanonoff]{Fan:OnOff}, \hyperref[fanvariablevolume]{Fan:VariableVolume}, and \hyperref[fanperformancenightventilation]{FanPerformance:NightVentilation}. The design electric power can be entered directly or autosized using one of three methods. For continuous speed control, a performance curve or table describes electric power as a function of flow fraction. For discrete speed control, the power fraction at each speed can be entered directly or calculated from that curve or table.
...
The name of an availability schedule for this fan. Schedule values greater than zero mean that the fan is available. If this field is blank, the fan is always available.
...
This numeric field is the design volume flow rate of the fan as installed in the HVAC system, in m\(^{3}\)/s. This volume flow rate is converted to a mass flow rate using the altitude-adjusted standard density of dry air at \SI{20}{\celsius} dry-bulb temperature. This is the full-speed flow rate and is the upper limit for fans that can vary their flow rate. This field can be autosized.

\paragraph{Field: Speed Control Method}\label{field-speed-method-fansysmodel}

This field selects how the fan speed is controlled. The choices are Continuous and Discrete; the default is Discrete. Discrete control operates at one or more specified speed levels. When an intermediate flow is required, the model time-weights operation between adjacent speed levels. Continuous control allows the flow rate to vary smoothly up to the Design Maximum Air Flow Rate and calculates electric power from the Electric Power Function of Flow Fraction Curve Name. A constant-volume or on-off fan should use Discrete with Number of Speeds set to 1. A variable-air-volume fan should use Continuous.

\paragraph{Field: Electric Power Minimum Flow Rate Fraction}\label{field-power-min-flow-fraction-fansysmodel}

This numeric field is used to calculate the power of a continuously controlled fan at low flow rates; it does not impose a minimum air flow during simulation. The curve input is limited to no less than this fraction of Design Maximum Air Flow Rate. The value must be between 0 and 1, the default is 0.2, and it is used only when Speed Control Method is Continuous.
...
This numeric field is the electric power consumption at the full Design Maximum Air Flow Rate and Design Pressure Rise, in Watts. The entered or autosized value is also used to determine the fan efficiency. This field is autosizable and defaults to autosize. When it is autosized, the following field selects one of three sizing methods.
...
This numeric field is used when the Design Power Sizing Method is set to TotalEfficiencyAndPressure and the Design Electric Power Consumption is set to Autosize. It determines the Design Electric Power Consumption from the Design Maximum Air Flow Rate and Design Pressure Rise: Design Electric Power Consumption = (Design Maximum Air Flow Rate) * (Design Pressure Rise) / (Fan Total Efficiency). The default is 0.7.

\paragraph{Field: Electric Power Function of Flow Fraction Curve Name}\label{field-power-curve-name-fansysmodel}

This field is the name of a performance curve or table that describes how electric power consumption varies with air flow rate. The independent ``x'' variable is a normalized flow fraction defined as the current flow rate divided by the Design Maximum Air Flow Rate. The model uses the ratio of moist-air mass flow rates, so the numerator accounts for humidity and barometric pressure. The curve or table output is a fraction that is multiplied by the Design Electric Power Consumption. Any single-independent-variable curve can be used. This field is required when Speed Control Method is Continuous. It is also used with Discrete control when Number of Speeds is greater than 1 and a Speed ``n'' Electric Power Fraction is left blank. The fourth-order polynomial in \hyperref[curvequartic]{Curve:Quartic} can be used with the coefficients listed above to reproduce the formulation used by the older \hyperref[fanvariablevolume]{Fan:VariableVolume} object prior to version 8.6.

\paragraph{Field: Night Ventilation Mode Pressure Rise}\label{field-nite-vent-pressure-fansysmodel}

This optional numeric field is the total system pressure rise experienced by the fan when operating in night ventilation mode using \hyperref[availabilitymanagernightventilation]{AvailabilityManager:NightVentilation}, in Pascals. A positive value overrides the Design Pressure Rise during night ventilation; if this field is blank or zero, the design pressure rise is used. This field allows the fan to be modeled with the different system pressure that can occur when outdoor air is used to precool a building with dampers fully open. This field and the next one replace the \hyperref[fanperformancenightventilation]{FanPerformance:NightVentilation} object, which is not needed with this fan.

\paragraph{Field: Night Ventilation Mode Flow Fraction}\label{field-nite-vent-flow-fraction-fansysmodel}

This numeric field is the fan flow fraction for night ventilation mode. It corresponds to Maximum Flow Rate in the \hyperref[fanperformancenightventilation]{FanPerformance:NightVentilation} object. The input is accepted but is not currently used; \hyperref[availabilitymanagernightventilation]{AvailabilityManager:NightVentilation} uses its own flow fraction. This value must be between 0 and 1 and represents the fan flow relative to the Design Maximum Air Flow Rate.
...
This integer field specifies the number of speed levels available when Speed Control Method is Discrete. The default is 1. This field and the remaining field sets are not used when Speed Control Method is Continuous. A value of 1 operates the fan at its maximum design speed, and no additional field sets are needed. For a value greater than 1, provide one pair of flow and power fractions for each speed in the remaining input fields.

\paragraph{Field Set: (Speed Flow Fraction, Speed Electric Power Fraction)}\label{fieldset-flow-power-fractions-fansysmodel}

A field set is a pair of values for the flow fraction and electric power fraction at each speed. Arrange the sets in increasing order of flow fraction. Typically, the highest speed matches the design maximum and has flow and power fractions of 1.0. This object is extensible, so additional sets of the following two fields can be added to the end of the object.
...
This field is the electric power fraction for the fan speed. It is multiplied by the Design Electric Power Consumption to obtain the power consumption at this speed. If this field is blank and Electric Power Function of Flow Fraction Curve Name is provided, the curve or table determines the power fraction. An entered value overrides the curve or table for this speed, so a separate performance curve is not required when power fractions are entered for all discrete speeds.
...
The pressure rise in Pascals at full flow and standard (sea level) conditions (\SI{20}{\celsius} and 101325 Pa).

\paragraph{Field: Maximum Flow Rate}\label{field-maximum-flow-rate-000}

The full load air volumetric flow rate (m\(^{3}\)/sec) at standard temperature and pressure (dry air at \SI{20}{\celsius} drybulb). The program does use local barometric pressure to account for altitude using equation for ``standard atmospheric'' pressure on p 6.1 of the ASHRAE 1997 HOF (SI edition) to initialize the air systems being simulated.

\begin{equation}
p = 101325 \left(1 - 2.25577 \times 10^{-5} Z\right)^{5.2559}
\end{equation}

where \(p\) is pressure in Pa and \(Z\) is altitude in m.
...
The constant coefficient (\(C_1\)) in a fourth order polynomial curve giving the fraction of full load power (PLF) as a function of flow fraction (FF). Flow fraction is the air mass flow rate divided by the maximum air mass flow rate. The curve is:

\begin{equation}
\mathrm{PLF} = C_1 + C_2 \cdot \mathrm{FF} + C_3 \cdot \mathrm{FF}^2 + C_4 \cdot \mathrm{FF}^3 + C_5 \cdot \mathrm{FF}^4
\end{equation}

\paragraph{Field: Fan Power Coefficient 2}\label{field-fan-power-coefficient-2}

The linear coefficient (\(C_2\)) in a fourth order polynomial curve giving the fraction of full load power (PLF) as a function of flow fraction (FF). Flow fraction is the air mass flow rate divided by the maximum air mass flow rate. The curve is:
...
The quadratic coefficient (\(C_3\)) in a fourth order polynomial curve giving the fraction of full load power (PLF) as a function of flow fraction (FF). Flow fraction is the air mass flow rate divided by the maximum air mass flow rate. The curve is:
...
The cubic coefficient (\(C_4\)) in a fourth order polynomial curve giving the fraction of full load power (PLF) as a function of flow fraction (FF). Flow fraction is the air mass flow rate divided by the maximum air mass flow rate. The curve is:
...
The quartic coefficient \(C_5\) in a fourth order polynomial curve giving the fraction of full load power (PLF) as a function of flow fraction (FF). Flow fraction is the air mass flow rate divided by the maximum air mass flow rate. The curve is:
...
This field is optional. If it is not used then there will be no temperature-related control over the operation of the exhaust fan. If the field is used, then enter the name of a schedule with values for zone temperature values (\si{\celsius}). The fan's control will be based on a comparison between the current zone air temperature and the schedule values. If the zone is warmer than the scheduled limit, then the fan will operate. When balancing with simple ventilation, this feature can be used to coordinate exhaust fan operation with ZoneVentilation:* controls for minimum indoor temperature.
...
The pressure rise in Pascals at full flow and standard (sea level) conditions (\SI{20}{\celsius} and 101325 Pa). This is a required field with no default.
...
Fan:ComponentModel is a detailed fan model for central constant-air-volume (CAV) and variable-air-volume (VAV) air systems. Its inputs describe the air-distribution system, fan wheel, drive belt (if used), motor, and variable-frequency drive (VFD, if used). See the Engineering Reference for the model equations and additional background.
...
The optional name of the schedule that determines whether the fan can run. A schedule value of 0 turns the fan off, and a value greater than 0 allows it to operate. If this field is blank, the fan is always available. Applicable availability managers (see \hyperref[availabilitymanagerassignmentlist]{AvailabilityManagerAssignmentList}) may override this schedule by forcing the fan on or off.

\paragraph{Field: Maximum Flow Rate}\label{field-maximum-flow-rate-5}

The full-load volumetric airflow (m\(^{3}\)/sec) through the fan at standard temperature and pressure (dry air at \SI{20}{\celsius} dry-bulb). To initialize the air systems being simulated, the program uses local barometric pressure adjusted for altitude, based on the equation for ``standard atmospheric'' pressure on p.6.1 of the 1997 ASHRAE Handbook of Fundamentals (SI edition):

\begin{equation}
p = 101325 \left(1 - 2.25577 \times 10^{-5} Z\right)^{5.2559}
\end{equation}

where \(p\) is pressure in Pa and \(Z\) is altitude in m. Can be autosized.
...
where \(\Delta P_{fan}\) is fan static pressure rise (Pa; see the \emph{Fan Pressure Rise Curve Name} field), \(D_{fan}\) is wheel diameter (m), \(\rho\) is the manufacturer's reference air density (kg/m\(^{3}\)), and \(Q_{fan}\) is airflow through the fan (m\(^{3}\)/sec). \(Eu_{max}\) is calculated using a pressure-rise and airflow pair that corresponds to maximum fan static efficiency for the specified fan. It must be greater than zero.

\paragraph{Field: Maximum Dimensionless Fan Airflow}\label{field-maximum-dimensionless-fan-airflow}

The required numeric maximum dimensionless airflow (\(\varphi_{max}\)) through the fan, which corresponds to the maximum ratio between fan airflow (\(Q_{fan}\), m\(^{3}\)/sec) and fan shaft rotational speed (\(\omega_{fan}\), rpm) for the specified fan wheel diameter (\(D_{fan}\), m). \(\varphi_{max}\) is determined from manufacturer data using:
...
The numeric dimensionless factor (\emph{F\(_{belt}\)}) used to multiply the specified or autosized maximum belt output torque (\(\tau_{belt,max}\)). If specified, the minimum value is 1.0. The default is 1.0.
...
The curve output is the natural logarithm of efficiency and must be between -4.6 and 0.0. The model exponentiates that output to obtain a maximum belt efficiency greater than zero and less than or equal to 1.0. If \(\eta_{\mathrm{belt, max}}\) is known, set \(C_1 = ln(\eta_{\mathrm{belt, max}})\) and set \(C_2\) through \(C_5\) to zero. If this field is blank, such as for a direct-drive fan, the model uses a maximum belt efficiency of 1.0.
...
The alpha name of the single rectangular hyperbola type 1 performance curve (ref: \hyperref[curverectangularhyperbola1]{Curve:RectangularHyperbola1} in Performance Curves) that determines the maximum fan motor efficiency (\(\eta_{\mathrm{motor, max}}\)) as a function of \(x_{\mathrm{motor, max}}\). The curve is:
