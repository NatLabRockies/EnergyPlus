...
\paragraph{Field: Number of Timesteps per Hour for Sizing}\label{field-number-of-timesteps-per-hour-for-sizing}

The previous field for Number of Timesteps per Hour specifies the ``basic'' timestep for the simulation. This value defines the Timestep used during the sizing simulation. This value will be used in the Zone Heat Balance Model calculation as the driving timestep for heat transfer and load calculations only during the sizing periods. If this field is blank the default value is 6. If this field is not included in the input then the Number of Timesteps per Hour for Sizing is equal to the basic Number of Timesteps per Hour. After sizing is complete the value in the previous field will be used. The allows the user to specify a different time step for the sizing simulation than used during the actual building simulation. Longer length timesteps have lower values for Number of Timesteps per Hour for Sizing. For example a value of 6 entered here directs the program to use a zone sizing timestep of 10 minutes and a value of 60 means a 1 minute timestep. The user's choice for Number of Timesteps per Hour for Sizing must be evenly divisible into 60; the allowable choices are 1, 2, 3, 4, 5, 6, 10, 12, 15, 20, 30, and 60.

There is also third type of timestep inside EnergyPlus that is known as the System Timestep. This is a variable-length timestep that governs the driving timestep for HVAC and Plant system modeling. The user cannot directly control the system timestep (except by use of the \hyperref[convergencelimits]{ConvergenceLimits} object). When the HVAC portion of the simulation begins its solution for the current zone timestep, it uses the zone timestep as its maximum length but then can reduce the timestep, as necessary, to improve the solution. The technical details of the approach are explained in the Engineering Documentation under ``Integrated Solution Manager''.
