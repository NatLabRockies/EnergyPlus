...
\paragraph{Field: Design Water Pump Power Per Unit Flow Rate}\label{field-design-water-pump-power-per-unit-flow-rate}

This numeric field value is recirculating water pump sizing ratio in \si{\wattperVolumeFlowRate}.
...
Average water pump power per unit primary design air flow rate was estimated from pump power and primary air design flow rate inputs from energyplus example files and
...
\paragraph{Field: Design Water Pump Power Per Unit Flow Rate}\label{field-design-water-pump-power-per-unit-flow-rate-1}

This numeric field value is recirculating water pump sizing ratio in \si{\wattperVolumeFlowRate}. This field is used when the previous field is set to autosize. The pump design electric power is scaled with Secondary Design Air Flow Rate. Average water pump power per unit secondary design air flow rate was estimated from pump power and secondary air design flow rate inputs from energyplus example files and is about 90.0 \si{\wattperVolumeFlowRate} ( = 90.0 \textasciitilde{} Pump Power / Secondary Air Design Flow Rate). The factor ranges from 55.0 to 150.0 \si{\wattperVolumeFlowRate}.
...
The output variables that are available for the EvaporativeCooler:Indirect:UserEffectiveness object are shown below:
