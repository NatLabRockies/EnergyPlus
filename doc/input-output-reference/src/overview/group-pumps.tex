...
This object models a variable-speed pump on a plant or condenser loop.

\subsubsection{Inputs}\label{inputs-037}

\paragraph{Field: Name}\label{field-name-036}

This alpha field contains the unique identifying name for the pump.
...
This numeric field contains the pump motor's efficiency in decimal form. The value must be greater than 0 and less than or equal to 1. The default is 0.9.
...
This numeric field is the fraction of pump motor losses that is added to the fluid. The value must be between 0 and 1; the default is 0.
...
\begin{equation}
FractionFullLoadPower = C_1 + C_2 PLR + C_3 PLR^2 + C_4 PLR^3
\end{equation}

where \(C_1\), \(C_2\), \(C_3\), and \(C_4\) are Coefficients 1 through 4, and \(PLR\) is the part-load ratio. The defaults are 0, 1, 0, and 0, respectively.
...
This field contains the minimum volume flow rate while the pump is operating, in cubic meters per second. It can be autosized. When it is autosized, Design Minimum Flow Rate Fraction is multiplied by the Design Maximum Flow Rate. The minimum flow rate should be less than the maximum flow rate. If an entered minimum is greater than 99\% of the Design Maximum Flow Rate, a warning is issued and the minimum is reset to 99\% of the maximum.

\paragraph{Field: Pump Control Type}\label{field-pump-control-type-000}

The choices are Continuous and Intermittent; the default is Continuous. The maximum and minimum flow rates are the physical operating limits. An intermittent pump can shut down when there is no loop load. A continuous pump, or an intermittent pump when the loop has a load, operates between the minimum and maximum limits while trying to meet flow requests from demand-side components. Applicable availability managers (see \hyperref[availabilitymanagerassignmentlist]{AvailabilityManagerAssignmentList}) may override this control by forcing the pump on or off.

\paragraph{Field: Pump Flow Rate Schedule Name}\label{field-pump-flow-rate-schedule-name-000}

Each schedule value is limited to the range 0 to 1 and multiplies the Design Maximum Flow Rate. This sets the maximum flow available to the pump at that time, \textbf{not the actual flow rate}. If this field is blank, the pump follows its other operating requirements without this additional limit. This schedule is intended for special pump operation; EMS actuators can provide additional flow control.
...
This field selects the VFD control strategy. PressureSetpointControl uses the minimum and maximum pressure and rpm schedules to vary pump speed within those limits. ManualControl is an idealized strategy that maintains the scheduled pump rpm while respecting other flow restrictions in the loop.

\paragraph{Field: Pump RPM Schedule Name}\label{field-pump-rpm-schedule-name}

For VFD Control Type = ManualControl, this field defines the pump rpm schedule. Schedule values must be greater than zero. For VFD Control Type = PressureSetpointControl, this field is not used.

\paragraph{Field: Minimum Pressure Schedule}\label{field-minimum-pressure-schedule}

For VFD Control Type = PressureSetpointControl, this field defines the lower bound of the pressure drop (head) used to determine the required pump speed. For ManualControl, this field is not used. Schedule values are in Pascals.

\paragraph{Field: Maximum Pressure Schedule}\label{field-maximum-pressure-schedule}

For VFD Control Type = PressureSetpointControl, this field defines the upper bound of the pressure drop (head) used to determine the required pump speed. For ManualControl, this field is not used. Schedule values are in Pascals.

\paragraph{Field: Minimum RPM Schedule}\label{field-minimum-rpm-schedule}

For VFD Control Type = PressureSetpointControl, this field defines the minimum allowable rpm used when determining pump speed. For ManualControl, this field is not used.

\paragraph{Field: Maximum RPM Schedule}\label{field-maximum-rpm-schedule}

For VFD Control Type = PressureSetpointControl, this field defines the maximum allowable rpm used when determining pump speed. For ManualControl, this field is not used.
...
This field is optional. It is used when Design Power Consumption is autosized and Design Power Sizing Method is PowerPerFlow. The value scales design electric power directly from the design flow rate. If omitted, the default is 348701.1 W/(m\(^{3}\)/s) (22 W/gpm).
...
This field is optional. It is used when Design Power Consumption is autosized and Design Power Sizing Method is PowerPerFlowPerPressure. The value represents the inverse efficiency of the impeller and drive assembly when design power is calculated from design flow rate, pump head, and motor efficiency. If omitted, the default is 1.282051282 W/((m\(^{3}\)/s)-Pa).

\paragraph{Field: Design Minimum Flow Rate Fraction}\label{field-design-minimum-flow-rate-sizing-factor}

This field is optional and must be between 0 and 1. When Design Minimum Flow Rate is autosized, the minimum flow rate is the product of this fraction and the Design Maximum Flow Rate. The default is 0.0.
...
Pump Electricity Rate is the electric power input to the pump motor. Pump Electricity Energy is that power integrated over the reporting interval. The energy is metered on Pumps:Electricity, Electricity:Plant, and Electricity:Facility.
...
These outputs report the heat added to the fluid. The heat gain rate is the pump shaft power plus the fraction of motor inefficiency specified by Fraction of Motor Inefficiencies to Fluid Stream. The energy output is the heat gain rate integrated over the reporting interval.
